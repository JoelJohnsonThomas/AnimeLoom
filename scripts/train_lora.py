...
def train(
    ...
    class CharDS(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            stem = Path(p).stem
            cap = self.caps.get(stem, f"{character_name}, anime character portrait")
    ...
    while global_step < steps:
        for batch in dl:
            ...
            with torch.no_grad():
                enc_hidden = text_encoder(tok.input_ids.to(device))[0]
            ...
            # Predict
            pred = unet(noisy, ts, encoder_hidden_states=enc_hidden).sample
...
def main():
    ...
    p.add_argument("--base-model", default="stabilityai/stable-diffusion-2-1")
    args = p.parse_args()
