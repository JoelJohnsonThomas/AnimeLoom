...
if use_animatediff:
    ...
    for _model_id in _SD15_MODELS:
        try:
            anim_pipe = AnimateDiffVideoToVideoPipeline.from_pretrained(
                _model_id,
    ...
    if anim_pipe is None:
        ...
        for i, kf_image in enumerate(keyframes):
            ...
            gen = torch.Generator("cpu").manual_seed(42 + i)
            try:
                # Create static video from keyframe (AnimateDiff vid2vid adds motion)
...
checks = [
    ...
    ("AnimateDiff vid2vid pipeline", "AnimateDiffVideoToVideoPipeline" in cell28_src),
    ("MotionAdapter", "MotionAdapter" in cell28_src),
    ("vid2vid call pattern", "video=input_video" in cell28_src),
    ("SD 1.5 LoRA auto-train", "train_sd15_lora" in cell28_src),
    ("PeftModel LoRA loading", "PeftModel.from_pretrained" in cell28_src),
    ("DENOISING_STRENGTH param", "strength=DENOISING_STRENGTH" in cell28_src),
